...
void KatDataCore::Initialization()
{
	...
	FString filePath = FPaths::Combine(*FPaths::GamePluginsDir(), TEXT("KatDataPlugin4BP/ThirdParty/WalkerBase.dll"));
	if (filePath.IsEmpty())
	{
		filePath = FPaths::Combine(*FPaths::GamePluginsDir(), TEXT("WalkerBase.dll"));
	}
